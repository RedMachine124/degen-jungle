  ...
  try {
    ...
    const ranked = pairs
      ...
      .map(p => ({
        ...
        score: (Number(p.txns?.m5?.buys || 0) + Number(p.txns?.m5?.sells || 0)) * 5
              + Number(p.volume?.h1 || 0),
      }));
    // Deduplicate by mint, keep highest score
